<template>
  ...
  >
    <div
      class="tw-flex-grow sm:tw-flex-grow-0 md:tw-flex-grow-0 lg:tw-flex-grow-0 xl:tw-flex-grow-0 tw-p-0 tw-m-0 tw-max-h-cover tw-shadow"
      :style="
        'background: url(' +
          Songs[presentSongId].cover +
    ...
    >
      <div class="tw-relative tw-shadow tw-max-w-full play-controls">
        <div class="tw-flex tw-flex-col tw-items-center tw-p-0 tw-m-0">
          <div
            class="tw-flex-grow tw-h-full tw-max-h-coverx tw-w-full tw-m-3 tw-items-center tw-align-middle tw-mt-1 tw-mx-auto tw-mb-3 tw-p-2"
          >
            <v-circle
          ...
          <div
            class="tw-inline-flex tw-px-4 tw-py-1 tw-mb-2 tw-align-middle tw-justify-around pctrl tw-rounded-full"
          >
            <div class="tw-mx-4">
              <span @click="prevSong()"
                ><SkipBackwardIcon
            ...
            <div class="mx-4">
              <span @click="play()"
                ><PlayIcon
                  class="tw-text-white tw-cursor-pointer"
                  v-show="!isPlaying && !playerIsBuffering"
                  w="30"
                  h="30"
              /></span>
              <span @click="play()"
          ...
          <div class="tw-flex-grow track-scrubbing tw-mx-5 pctrl">
            <vue-slider
              :width="260"
              @change="scrubChange()"
              v-model="playerProgressPercent"
              :process="true"
              :interval="0.01"
            ></vue-slider>
          </div>
          <div
            class="tw-inline-flex tw-items-start tw-px-2 tw-mx-0 tw-w-full tw-align-middle tw-justify-around pctrl"
          >
            <div class="tw-inline-flex tw-items-center tw-align-middle tw-m-1">
              ...
              <span
                v-tooltip.top-center="
                  continuousPlay ? 'Repeat: ALL' : 'Repeat: OFF'
                "
                @click="changeContinuousPlay()"
                :class="
                  continuousPlay
                    ? 'tw-p-1 tw-bg-transparent tw-text-primary-green'
                    : 'tw-p-1 tw-bg-transparent tw-text-white'
                "
                ><RefreshIcon
              /></span>
            </div>
            <div class="tw-p-2">
              <div class="tw-text-sm tw-text-white">
                {{ timeBufferMins }} : {{ timeBufferSecs | doubleDigits }} -
    ...
    <div
      class="tw-flex-grow tw-pt-0 tw-pl-0 sm:tw-pl-2 tw-mt-2 sm:tw-mt-0 tw-ml-0 sm:tw-ml-2"
    >
      <!--<progress-bar bar-color="#333333" size="tiny" :val="audio.currentTime" :max="!audio.duration ? 100 : audio.duration"></progress-bar>-->
      <songs-playlist
        :current-track-id="presentSongId"
        @playSelectSong="playSong"
        :Songs="Songs"
      ></songs-playlist>
    </div>
  </div>
