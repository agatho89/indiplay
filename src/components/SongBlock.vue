<template>
  ...
  >
    ...
    {{ currentTrackId === count ? "" : "" }}
    <span
      :style="'background: rgba(0,255,0, .2); width: ' + progressPercent + '%;'"
      :class="
        currentTrackId === count
          ? 'tw-absolute tw-top-0 tw-bottom-0 tw-left-0 tw-right-0 tw-p-0 tw-m-0 tw-h-full tw-w-full tw-z-30 tw-bg-green-100 tw-h-full'
          : 'tw-hidden'
      "
    ></span>
    <span
      class="tw-absolute tw-py-auto tw-pr-2 tw-right-0"
      v-if="currentTrackId === count && playerIsBuffering"
      ><BufferingIcon
        animate="beat"
...
export default {
  ...
  computed: {
    ...mapState(["progressPercent", "playerIsBuffering"])
  }
};
</script>
